...
function getNextFriday(date) {
  ...
  newDate.setDate(newDate.getDate() + ((5 - newDate.getDay() + 7) % 7));
...
function getWeekNumberByDate(date) {
  ...
  const daysOffset = yearStart.getDay();
  const diffInDays = Math.floor((newDate - yearStart) / 86400000);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (workDaysCount <= countWorkDays) {
      result.push(currentDate.toLocaleDateString().split('.').join('-'));
      workDaysCount += 1;
